...
function particle_filter(trial::Scene,
    ...
    # Generative model is initialized using trial constants
    model = make_model(trial)
    # ess is set to the number of particles * fact
    ...
    for (it, (t, active)) in enumerate(args)
        ...
        # retrieve the list of random choices associated with active objects
        ...
        # apply a rejuvenation move to each particle for each
        # random choice associated with an active object
