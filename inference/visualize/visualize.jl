...
function draw_predictions(predictions)
    num_sids = first(size(predictions))
    for sid = 1:num_sids
        sethue("blue")
        Luxor.circle(predictions[sid, 1, 1]*scale_fac, predictions[sid, 1, 3]*scale_fac,
                     2.5, :stroke)
        sethue("black")
        Luxor.circle(predictions[sid, 2, 1]*scale_fac, predictions[sid, 2, 3]*scale_fac,
                     2.5, :stroke)
    end
end
...
function visualize(gt,
    ...
    function frame(scene, framenumber)
        ...
        draw_predictions(predictions[:, framenumber, :, :])
    end
    animate(mov,
