...
from stimuli.simple_tower_search NoisyStability
...
def evaluate_tower(base, size, gen, phys, pred, out, noise, debug = False):
    ...
    others = list(gen.configurations(tower))
    # compute stats over towers
    stats = phys(tower, configurations = others)
    # evaluate metrics
    orig = stats[0]
    columns = ('id', 'angle', 'mag', 'instability')
    if noise:
        columns += ('instability_p',)

    df = pd.DataFrame(stats[1:], columns = columns)
    df = df.apply(add_cols, axis = 1)

    new_df, metric  = pred(df, orig)
    # Don't filter in debug
    if not debug:
        df = new_df

    if len(df) == 0:
        # return if no configurations are interesting
        return (False, None)

    # Retrieve the mut name of the form <block id>_<mut type>
    winner_idx = df[metric].abs().idxmax()
    winner_id = df.loc[winner_idx, 'id']
    block_id = int(df.loc[winner_idx, 'block'])
    mut = df.loc[winner_idx, 'mut']
    winning_tower = others[block_id - 1][mut]
    # Package results for json
    result = {
        'org-tower' : {
            'struct': mutate_tower(tower, block_id - 1).serialize(),
            'stats' : {k: orig[k] for k in columns},
            'trace' : orig['trace'],
        },
        'mut-tower' : {
            'struct' : mutate_tower(winning_tower, block_id - 1).serialize(),
            'stats'  : {k: stats[winner_idx + 1][k] for k in columns},
            'trace' : stats[winner_idx + 1]['trace'],
        },
        'block' : block_id,
        'mutation' : mut,
        'metric' : metric,
        'metric_delta' : df.loc[winner_idx, metric]
    }
...
def make_stab_pred(args):
    ...
    thresh = args.thresh
    def pred(df, orig):
        df = df[df['mut'] == args.weight]
        if orig['instability'] > 0.0:
            df = df[df['instability'] == 0]
            if len(df) == 0:
                return (df, metric)
        df.loc[:, metric] = df[mode] - orig[mode]
        print(df)
        df = df[(df[metric] * args.delta) >= thresh]
        return df, metric
    return pred

def make_angle_pred(args):
    mode = 'angle'
    metric = 'angle_diff'
    thresh = np.pi / 3.
    def pred(df, orig):
        df = df[df['mut'] == args.weight]
        df = df[df['instability'] > 0]
        if orig['instability'] == 0.0 or len(df) == 0:
            return (df, metric)
        v = orig[mode] - df[mode]
        df.loc[:, metric] = v
        df = df[df[metric].abs() >= thresh]
        return df, metric
    return pred

def stab_path(args):
    delta = 'low' if args.delta < 0 else 'high'
    return 'instability_{}_{}'.format(args.weight, delta)
def angle_path(args):
    return 'angle_{}'.format(args.weight)
...
def main():
    ...
    parser_stab = subparsers.add_parser('instability',
                                        help='Search for stability pairs')
    parser_stab.add_argument('weight', type=str, choices = ['L', 'H'],
                             help = 'Which mutation to search over')
    parser_stab.add_argument('delta', type=int, choices = [-1, 1],
                             help = ('Whether the mutation increases'+\
                             ' or decreases stability'))
    parser_stab.add_argument('--thresh', type=float, default = 0.4,
                             help = ('Change in instability'))
    parser_stab.set_defaults(search=make_stab_pred, make_path = stab_path)

    parser_angle = subparsers.add_parser('angle',
                                        help='Search for angle pairs')
    parser_angle.add_argument('weight', type=str, choices = ['L', 'H'],
                             help = 'Which mutation to search over')
    parser_angle.set_defaults(search=make_angle_pred, make_path = angle_path)
    ...
    enough_scenes = lambda l: len(np.argwhere(l[0] == '')) == 0
    ...
    if args.noise > 0:
        ...
        phys = ExpStability(noise = args.noise, frames = 240)
